 ...
 * 金额识别模式 —— **唯一真相**。
 *
 * 三种合法形态（必须是「货币语境」中的数字，纯整数不算）：
 * 1. 带货币符号：`¥12.00` / `￥1,234`
 * 2. 带小数点：  `12.00` / `1,234.56`
 * 3. 带「元」后缀：`12元` / `1,234.56 元`
 *
 * 这条约束是整套解析最值钱的一段：它保证「您账户2836于…」里的卡号尾号 2836
 * 不会被当成金额。S1 [com.kai.bill.data.parser.AmountGate] 直接复用本常量，
 * 不在别处再写一份金额正则 —— 两份迟早会不一致，而不一致的结果就是错账。
 */
object AmountPattern {
     ...
     * 金额识别正则。
     *
     * 三种合法形态（必须落在「货币语境」里，纯整数不算）：
     * 1. 带货币符号：`¥12.00` / `￥1,234`
     * 2. 带小数点：  `12.00` / `1,234.56`
     * 3. 带「元」后缀：`12元` / `1,234.56 元`
     *
     * 这条约束是整套解析最值钱的一段：它保证「您账户2836于…」里的卡号尾号 2836
     * 不会被当成金额。取第一个匹配即可（见 [com.kai.bill.data.parser.AmountGate]），
     * 不在别处再写一份金额正则 —— 两份迟早会不一致，而不一致的结果就是错账。
     */
    val ANY: Regex = Regex(
...
/**
 * 方向级词表（P0：代码常量；P1 迁入 `match_keyword` 表后由仓储提供，阶段类逻辑不变）。
 *
 * **判定顺序由 [com.kai.bill.data.parser.DirectionRouter] 保证**，本文件只声明数据：
 *
 * 1. [exclude] 优先于一切 —— 失败 / 预告 / 营销通知也带金额，不拦住就会记成真实收支；
 * 2. 还款类（[MatchRoute.TRANSFER]）直接落库、不计统计；
 * 3. 具体收支动作词（[MatchRoute.EXPENSE] / [MatchRoute.INCOME]）计入统计；
 * 4. 泛化入账（[MatchRoute.GENERIC_INBOUND]）与自己的钱搬家 / 红包 → 待确认。
 *
 * 优先级约定：`0` = 常规，`-10` = **泛化词**（仅在没有任何具体词命中时才采用），
 * `10` = 强词（极端明确、必须压过泛义词）。同级比较时**更长的词获胜**（更具体者胜）。
 */
object DefaultMatchKeywords {
    ...
    /**
     * 泛化入账 / 泛化交易：**只在没有具体词命中时**才被采用。
     *
     * 为什么必须低于具体词：银行通知里「入账」既可能是别人给你钱（真实收入），
     * 也可能是你把支付宝余额提现到这张卡（自己的钱搬家）。没有具体词兜底时
     * 一律交人工确认，宁可不记账也不虚增收入。
     */
     ...
     *
     * 这一类是**旧实现完全没有的**，也是错账的主要来源之一：
     * - 失败类：`支付宝 支付失败 ¥88.00` 曾被记成一笔真实支出；
     * - 预告类：`信用卡账单已出，本期应还 ¥1,200` 曾被记成支出 / 转账；
     * - 营销类：`满50元减10元` 曾被记成支出；
     * - 申请中：`退款申请已提交，预计 3 天到账 ¥29.80` 曾被提前记成收入。
     */
    ...
    val direction: List<DirectionKeyword> = buildList {
        ...
        // —— 转账：个人转账（微信 / 支付宝 的转账详情页）——
        // 这两句是转账详情页的固定文案，分别对应两个方向：
        // - `<对方>已收款 ¥200`            → **我付出去**，对方收到了；
        // - `你已收款，资金已存入零钱 ¥100` → **我收进来**。
        // 但**两者都不是真实收支**：钱只是从一个人手里到另一个人手里，与「还款」同类，
        // 所以归 [MatchRoute.TRANSFER] —— 照常落库供用户核对，但**不计入统计**，
        // 否则「朋友转我 200」会被算成这个月赚了 200。
        //
        // ⚠️ 「已收款」必须显式收进来：不收的话，`周建鑫已收款 ¥200` 会命中其中更短的
        // 「收款」→ 收入，凭空多算一笔收入（真机已复现）。[KeywordMatcher.best] 同级比长度，
        // 长词天然胜出，所以这一个词就足以压过泛化的「收款」而不用动那条规则。
        addAll(
            listOf(
                "已收款", "资金已存入零钱"
