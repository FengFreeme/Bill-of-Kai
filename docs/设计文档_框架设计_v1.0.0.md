...
│   │           ├── KaiProgressBar.kt      # 预算进度条（按进度绿→琥珀→鲜红）
...
**`isRefund` 设计说明（v2 → v3 新增）**：退款不是「一笔新的收入」，而是**同一笔支出的逆操作**，
因此它单独占一个字段，而不是靠分类或类型区分：

| 场景 | `type` | `isRefund` | 效果 |
|---|---|---|---|
| 京东买 90，三天后全额退 | `INCOME` | `true` | **支出侧 -90**（冲抵到原消费的月份与分类），收入侧不计入 |
| 报销到账 200 | `INCOME` | `false` | 真实进账，照常计入收入 |
| 手动记一笔退款（选「退款」分类） | `INCOME` | `true` | 同上；冲抵落在当月与本分类（手动账没有原消费可回溯） |
| 编辑一笔退款、把类型改成支出 | `EXPENSE` | `false` | 改类型即清标记，否则这笔支出会按负值参与统计 |

两条口径约定（改任一处都要同步）：
1. **带符号金额**只在 `Bill.signedExpenseCents` / `Bill.signedIncomeCents` 定义一次；
   `StatsDao` 里是等价的 SQL 写法（`CASE WHEN isRefund = 1 THEN -amountCents ...`），
   `StatsAggregator` 是等价的内存写法 —— 三处必须一致，否则「带筛选」与「不带筛选」会算出不同的数；
2. **归属**（`time` / `categoryId` / `accountId`）在落库时就改写成**原消费**那一笔的值
   （采集链路按「同金额 + 更早 + 最近一笔」回溯，窗口 30 天），因此
   「哪个月支出的就哪个月加回去」；回溯不到时退回自身，口径仍自洽。
