...
enum class ReviewCardReason {
    ...
    /** 已有账单，但这次识别**没定出分类**，等用户自己选（`ReconcileOutcome.NO_MATCH`） */
    NO_MATCH
}
