 ...
 *
 * 自绘而非 Vico：这里要的是「引线 + 名称 + 占比」的外置标签与圆心文字，Vico 的饼图偏向纯图形，
 * 硬套要绕过它自己的标签布局，而扇区只有十几个，自绘的可控性更划算。
 *
 * 引线固定两段（径向引出 → 水平进入标签），标签高度由角度决定且约束在引线所属象限内
 * （见 [layoutLabels]）；占比低于 [MIN_LABEL_RATIO] 的扇区默认不标注，点击选中后才出现。
 * 文字用 [rememberTextMeasurer] 量好再画，避免中文字宽偏差导致标签出界。
 *
 * @param centerTitle 圆心主文案（如金额）
 * @param selectedIndex 当前高亮的扇区下标，-1 表示无高亮
 * @param onSliceClick 点击扇区回调；调用方负责「再次点击取消高亮」
 */
@Composable
fun DonutChart(
...
/**
 * 排布引线标签。三条规则：
 * 1. 标签留在引线所属象限（否则线会折到对角，看图的人得满屏找对应关系）；
...
/**
 * 同象限纵向防撞：按「左/右 × 上/下」分四组**分别**处理，绝不跨象限挪动
 * （跨象限会破坏「标签留在自己引线所在象限」这条规则）。
 *
 * 做法：被压住的标签往下推；若整体被推出该象限下边界，则整体回拉、从下到上收紧。
 */
 ...
 *
 * 折点取「径向末端」与「该高度上圆环外沿」中更外侧者，并夹在径向末端与标签终点之间，
 * 保证两段线都只朝环外走、不管标签被防撞推到哪里都不会折回来；折点正好落在径向末端时
 * 直接一条斜线连进标签，省掉多余的小折角。高亮扇区的引线用扇区本色并加粗。
 */
...
/**
 * 子分类构成的亮色调色板：不能用分类自身的 `colorHex` —— 子分类的颜色继承自父分类，
 * 「餐饮」下 5 个子分类会是同一个色值、画出来无法区分。按扇区顺序取色，整体偏高明度。
 */
...
/**
 * 按金额降序的渐变色系：最大扇区深蓝，依次过渡到浅蓝、青、绿、黄、橙。
 *
 * 在 HSV 的 222°(蓝) → 28°(橙) 之间线性插值，饱和度高、明度略降，
 * 整体从冷到暖且相邻扇区有区分度。
 *
